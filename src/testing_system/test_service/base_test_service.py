...
class BaseTestService:
    ...
    def get_moderator_course_ids(self, user_id: int) -> List[int]:
        statement = select(tables.Moderators.course_id).filter_by(user_id=user_id)
        return self.session.execute(statement).scalars().all()
    ...
    def check_for_moderator_rights(self, user_id: int, course_id: int, course_owner_id: int):
        if course_id not in self.get_moderator_course_ids(user_id) and course_owner_id != user_id:
